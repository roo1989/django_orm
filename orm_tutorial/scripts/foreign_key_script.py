...
def run():
    ...
    restaurant = Restaurant.objects.prefetch_related('ratings').filter(ratings__isnull=True).first()

    print(f"restaurant: {restaurant.name} rating is {restaurant.ratings}")
    pprint(connection.queries)
